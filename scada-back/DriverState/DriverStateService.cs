...
public class DriverStateService : IDriverStateService
{
    ...
    public DriverStateDTO Create(DriverStateDTO dto)
    {
        ...
        if (existingDriverState != null) {
            throw new ObjectNameTakenException($"State with address '{dto.IOAddress}' already exists.");
        }
    ...
    public DriverStateDTO Get(int address)
    {
        ...
        if (state == null)
        {
            throw new ObjectNotFoundException($"State with address '{address}' not found.");
        }
        return state.ToDto();
    }
}